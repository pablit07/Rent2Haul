...
{  
    ...
    {  var normalizedPhone = stripCharsInBag(theField.value, phoneNumberDelimiters)
	  if (!isUSPhoneNumber(normalizedPhone, false)) 
          ...
          //alert(warnInvalid (theField, iUSPhone));
       else 
...
*/
function isValidURL(theUrl){
  if(theUrl.match(/^\w+([\.\-]\w+)*\.\w{2,4}(\:\d+)*([\/\.\-\?\&\%\#]\w+)*\/?$/i) || theUrl.match(/^(http|https)\:\/\/\w+([\.\-]\w+)*\.\w{2,4}(\:\d+)*([\/\.\-\?\&\%\#]\w+)*\/?$/i)){
    return true;
  } else {
    return false;
  }
...
function wordCounter(input_field, maxlimit)
{
	var wordcounter=0;
	for (var x=0; x<input_field.value.length; x++) {
		if (input_field.value.charAt(x) == " " && input_field.value.charAt(x-1) != " ")  {
			wordcounter++ ;
		}  
		if (input_field.value.charAt(x) == "\n" && input_field.value.charAt(x-1) != "\n")  {
			wordcounter++ ;
		}  
		if (input_field.value.charAt(x) == "\r" && input_field.value.charAt(x-1) != "\r")  {
			wordcounter++ ;
		}  
		if (input_field.value.charAt(x) == " " && input_field.value.charAt(x-1) != "\n")  {
			wordcounter++ ;
		...
		if (input_field.value.charAt(x) == " " && input_field.value.charAt(x-1) != "\r")  {
			wordcounter++ ;
		}  
		if (input_field.value.charAt(x) == "\r" && input_field.value.charAt(x-1) != " ")  {
			wordcounter++ ;
		}  
		if (input_field.value.charAt(x) == "\n" && input_field.value.charAt(x-1) != "\r")  {
			wordcounter++ ;
		}  
		if (input_field.value.charAt(x) == "\r" && input_field.value.charAt(x-1) != "\n")  {
			wordcounter++ ;
		}  
		if (wordcounter > maxlimit-1) {
			input_field.value = input_field.value.substring(0, x);
			alert("You can enter maximum "+maxlimit+" word(s)");
			return;
		}
	}
}
function charCounter(input_field, maxlimit)
{
	var charcounter=0;
	for (var x=0; x<input_field.value.length; x++) {
		charcounter++ ;
		if (charcounter > maxlimit-1) {
			input_field.value = input_field.value.substring(0, x-1);
			alert("You can enter maximum "+maxlimit+" character(s)");
			return;
		}
	}
}
function lineCounter(input_field, maxlimit, max_char_per_line)
{
	var linecounter=0;
	var spliter = '\r\n';
	var offset = 1;
	...
	var arr_line_wise = input_field.value.split(spliter);
	for (var y=0; y<arr_line_wise.length; y++) {
		if(arr_line_wise[y].length > max_char_per_line){
			arr_line_wise[y] = arr_line_wise[y].substring(0, max_char_per_line);
			input_field.value = arr_line_wise.join(spliter);
			alert("You can enter maximum "+max_char_per_line+" charecter per line");
			return void(0);
		}
	}
	...
	for (var x=0; x<input_field.value.length; x++) {
		...
		if (linecounter > maxlimit-1) {
			input_field.value = input_field.value.substring(0, x-offset);
			alert("You can enter maximum "+maxlimit+" line(s)");
			return void(0);
		}
	}
}
